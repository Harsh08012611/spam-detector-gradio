...
print("Please upload your spam dataset file when prompted...")
...
# Clean data
# 1. Drop rows with NaN in 'Category' or 'Message'
df = df.dropna(subset=['Category', 'Message'])
# 2. Rename columns
df = df.rename(columns={'Category': 'label', 'Message': 'message'})
# 3. Convert labels to numerical values, handling unknown labels
df['label'] = df['label'].str.lower().map({'ham': 0, 'spam': 1})
# 4. Drop rows where 'label' is NaN after mapping
...
# Step 9: AI-powered Email Classification Tool
